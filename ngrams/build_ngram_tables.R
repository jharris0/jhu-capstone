# Tokenize the training data and build ngram tables of most frequent ngrams.
# Note: On a machine with 16 GB of RAM, I needed to run this script in segments,
# i.e. I couldn't just source it and walk away. Generally, I would run each
# ngram tokenizing operation separately, and then run each table building
...
# Build ngram tables, starting with bigrams
bi_1 <- corpus_bigrams %>%
        separate(ngram, c("word1", "word2"), sep = " ")
setDT(bi_1)
setkey(bi_1, word1)
bi_2 <- bi_1[,.N, by = word1][order(-N)][1:75000] # Top 75000 bigrams
...
for(i in unique(bi_5$word1)) {
        ...
        tmp <- dt_tmp[1:4,.(word1, word2, N2, prob)] # Top 4 3rd words for each bigram
        bi_6[[i]] <- tmp
}
